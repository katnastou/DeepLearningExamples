...
if [ "$#" -ne 6 ]; then
    echo "Usage: $0 model_dir data_dir max_seq_len batch_size task init_checkpoint"
    exit 1
fi
...
cp ${OUTPUT_DIR}"/output_with_probabilities_dict.tsv" "/scratch/project_2001426/stringdata/pair-predictions-01/output_with_probabilities_"$(basename ${DATASET_DIR##*-})".tsv"

echo -n 'result written in /scratch/project_2001426/stringdata/pair-predictions-01'$'\n'
