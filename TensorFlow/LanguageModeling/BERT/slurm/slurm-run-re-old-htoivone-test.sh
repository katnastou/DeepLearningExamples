#!/bin/bash
# Definining resource we want to allocate. For multi-gpu We set 8 tasks, 4 tasks over 2 nodes as we have 4 GPUs per node.
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# Time limit on Puhti's gpu partition is 3 days i.e. 72:00:00.
### #SBATCH -t 04:00:00
#SBATCH -t 00:15:00

# Allocate 4 GPUs on each node for multi-gpu.
...
OUTPUT_DIR="output-biobert/relationextraction/$SLURM_JOBID"
mkdir -p $OUTPUT_DIR
...
DATASET_DIR=${2:-"scratchdata/4-class-10K-w20-htoivone_test"}
MAX_SEQ_LENGTH="$3"
BATCH_SIZE="$4"
LEARNING_RATE="$5"
EPOCHS="$6"
...
srun python run_re_consensus_htoivone_test.py \
    --do_prepare=true \
    --do_train=true \
    --do_eval=true \
    --do_predict=true \
