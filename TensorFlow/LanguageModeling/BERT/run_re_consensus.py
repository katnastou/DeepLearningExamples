...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, replace_span_A, replace_span_B):
    ...
    #replace [ ##unused ##3 ] with [unused3] before counting tokens
    ...
    if example.text_between_ent_1:
        tokens.extend(text_between_ent_1_tok)
        tokens.extend(equiv1_tok)
    ...
    if example.text_between_ent_2:
        tokens.extend(text_between_ent_2_tok)
        tokens.extend(equiv2_tok)
